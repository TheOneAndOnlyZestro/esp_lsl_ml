...
void run_app()
{
    ...
    for(int i =0; i < (int)(MODEL_COUNT / 4); i++)
    {
        ...
        snprintf(final_report + final_report_size, 7000 - final_report_size, "Model Window %ld\nQuant Type: Float32\n", WINDOW_LENS[i]);
        ...
        master_handle->dual_inference(&(X_DATA[X_OFFSETS[i]]), WINDOW_LENS[i], output_window, (int)(WINDOW_LENS[i] /2), &final_report[0], 7000);
        ...
        final_report_size = strlen(final_report);
        snprintf(final_report + final_report_size, 7000 - final_report_size, "MSE: %0.4f\n", mse);
        ...
        final_report_size = strlen(final_report);
        snprintf(final_report + final_report_size, 7000 - final_report_size, "=================================\n");
...
void allocateInputandOutputbuffers(float** input_buffer, float** output_buffer, float** correct_buffer, int** input_sizes, int** output_sizes, const uint8_t** data,int* total_input_size, int* total_output_size,int index)
{
    ...
    {
        *total_input_size += SIZES[index + j];
    }
...
void run_testing_benchmark()
{
    ...
    for(int i =0; i < (int)(BENCHMARK_MODEL_COUNT / 2); i++)
    {
        ...
        benchmark_handle->init_model(index, true, BENCHMARK_INPUT_SIZES[i], BENCHMARK_OUTPUT_SIZES[i]);
        ...
        benchmark_handle->init_model(index + 1, true, BENCHMARK_INPUT_SIZES[i], BENCHMARK_OUTPUT_SIZES[i]);
